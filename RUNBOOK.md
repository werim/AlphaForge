...
### Phase 8 PR 276 / PR 275 Reconciled Worker Flow
Use `start --foreground` for supervised PAPER collection, `start --detach` for background collection, and `worker --campaign-id` only as the subprocess entrypoint. Detached startup verifies that the worker remains alive; immediate exit marks the campaign failed.

Set `ALPHAFORGE_BURNIN_CAMPAIGN_ID` only for PAPER workers. The runtime attaches to the existing campaign active run and must not create a standalone Phase 7 run while campaign-bound. If heartbeat becomes stale, run `recover-check`; stale RUNNING campaigns transition to `RECOVERY_REQUIRED` and require explicit `resume`.

The worker resolver tick should run periodically with a canonical post-decision candle provider. Missing candles after horizon become `EXPIRED`; same-candle TP/SL reachability remains `AMBIGUOUS`.
