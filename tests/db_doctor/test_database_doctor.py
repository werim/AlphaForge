...
def test_repair_backs_up_and_certifies_real_writers(tmp_path):
    db=tmp_path/"repair.db"; command.upgrade(config(db),"0007_repair_runtime_lifecycle_schema")
    result=repair(db); assert result["status"] == "REPAIRED" and Path(result["backup_path"]).is_file()
    # Current bootstrap supplies all non-lifecycle writer-owned feature tables.
    canonical=tmp_path/"canonical.db"; init_db(f"sqlite+pysqlite:///{canonical}").dispose()
    certified=certify(canonical); assert certified["status"] == "DATABASE_CERTIFIED"
    assert {x["name"] for x in certified["writer_probes"]["checks"]} >= {"save_trade_lifecycle_event_signal_created","save_order_decision","runtime_heartbeat","runtime_state_snapshot"}
