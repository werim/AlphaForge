...
def test_campaign_provider_contamination_is_read_only_structural_failure(tmp_path):
    _, conn, cid = database(tmp_path)
    conn.execute("UPDATE rejected_signal_reviews SET symbol='BTCUSDT', payload_json=?",
                 (json.dumps({"campaign_id": cid, "symbol": "BTCUSDT",
                              "source_exchange": "hyperliquid"}),))
    conn.commit()
    before = conn.total_changes
    result = report(conn, cid)
    assert result["status"] == "FAIL"
    assert "CAMPAIGN_PROVIDER_MISMATCH" in result["reason_codes"]
