...
def test_reject_summary_surfaces_incomplete_rows(tmp_path) -> None:
    app = create_app(f"sqlite+pysqlite:///{tmp_path / 'rejects.db'}")
...
def test_lifecycle_timeline_is_sorted_and_flags_missing_reject_reason(tmp_path) -> None:
    app = create_app(f"sqlite+pysqlite:///{tmp_path / 'lifecycle.db'}")
    with app.state.engine.begin() as conn:
        conn.execute(text("""
...
def test_html_dashboard_pages_load(tmp_path) -> None:
