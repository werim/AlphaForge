...
def test_terminalization_revalidates_local_state_inside_immediate_transaction(monkeypatch, tmp_path, mutation, reason):
    ...
    def mutate():
        ...
        elif mutation == "pending_reject":
            persist_burnin_observation(other, observation_id="race-reject", burnin_run_id=run, release_id=camp.release_id, execution_mode="PAPER", decision="REJECTED", lifecycle_state="SIGNAL_REJECTED", metrics={"reject_decision_id":"race","campaign_id":camp.campaign_id})
            persist_pending_reject_label(other, campaign_id=camp.campaign_id, burnin_run_id=run, reject_decision_id="race", signal_id="s", symbol="BTCUSDT", side="LONG", decision_timestamp=utc_now(), entry=1, stop=.9, target=1.2, horizon_seconds=60, execution_cost_assumptions={}, regime="TREND", reject_reason="LOW_CONFIDENCE", source_provenance={})
