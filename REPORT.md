...
`src/alphaforge/remote_control/telegram_controller.py` coordinates only verified Telegram requests. `HELP` returns the currently executable Telegram command list (`/status`, `/health`, `/help`) with no executor, DB, subprocess, or runtime access. `STATUS` and `HEALTH` require an injected callable that receives only `RemoteControlCommand(name="STATUS", argv=("STATUS",))` or `RemoteControlCommand(name="HEALTH", argv=("HEALTH",))`. Unverified public request instances, tampered argv, unsupported commands, missing executors, executor exceptions, and invalid executor results return normalized fail-closed `RemoteControlDispatchResult` values without traceback/internal path/secret disclosure.
...
`tests/test_remote_control_telegram_controller.py` covers `HELP` output, no-executor `HELP`, exactly-once fake executor calls for verified `STATUS` and `HEALTH`, direct unverified request rejection before executor, duplicate update rejection before executor, unauthorized user/chat rejection before executor, normalized operation passing, unsupported command rejection before executor, safe executor failure, Telegram argument rejection before controller execution, tampered argv rejection, and no subprocess/network use.
