...
The P2-A golden pack declared full-chain expectations but exercised only a pre-submit projection. Initial frozen replays exposed campaign identity, accepted replay-finality and geometry evidence gaps; `489f495` repaired them and passed exact-head CI. The pack also lacked production-chain proof for accepted outcomes, execution rejects, portfolio limits and closed-candle geometry failures. Several reject fixtures contained stale gate names and evidence semantics: missing liquidity evidence was labeled as missing price geometry, execution rejects were labeled as portfolio blocks, thin liquidity omitted the preserved AIBrain primary reason, and generic `PORTFOLIO_OVEREXPOSURE` / `INCOMPLETE_REJECT_GEOMETRY` labels were used where production records a concrete risk state or decision reason.

## Change / behavior

`src/alphaforge/runtime.py` gives the attached campaign/run identity precedence over caller-supplied reject metadata. Twelve P2-A fixtures now carry frozen market input across accepted outcomes, ambiguous closure, execution gates, correlated portfolio exposure and incomplete closed-candle geometry. `tests/test_issue421_full_chain_replay.py` drives real AIBrain scoring, PAPER runtime, persisted lifecycle/decisions, resolver eligibility, readiness and isolated append-only audit ingestion twice per scenario. Reject replays preserve canonical geometry, omit false portfolio-block states, and create campaign-scoped pending labels only when forward-label geometry is eligible. Thin liquidity records `LOW_EXECUTION_PROBABILITY` as primary while retaining `THIN_LIQUIDITY`; the combined case records spread as primary and retains both failed gates. A seeded BTC LONG plus an ETH LONG candidate proves the concrete `CORRELATION_OVEREXPOSURE` decision and risk state. Incomplete closed-candle geometry records `GEOMETRY_INCOMPLETE` as the canonical reject, emits diagnostic completeness evidence and creates no pending label. Existing ambiguous, SIGKILL cold-start and cross-campaign identity regressions remain.

## Validation / impact

Twelve replay scenarios and 50 affected replay, portfolio and geometry tests passed. Accepted TP/SL paths preserve decision-to-position lineage and authoritative complete outcomes; ambiguous closure remains non-authoritative. Execution and portfolio rejects persist final decisions without execution. Incomplete geometry is durable and auditable but resolver-ineligible. Readiness remains blocked. No schema/CSV shape, migration, threshold, cost, LIVE authorization or active campaign database change. Previously persisted historical rows are untouched. The remaining 11 golden scenarios and bounded load/release qualification still need production replay. Push recommendation: CHATGPT only, followed by exact-SHA CI; LIVE NOT READY.
