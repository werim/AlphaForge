...
## 2026-08-01 — PAPER Control Center backend surgery report

The dashboard previously had burn-in presentation queries but no safe operational API tied to canonical campaign/continuation state. The patch adds a small router/service over the existing SQLite evidence and `alphaforge.burnin_cli`, not a parallel state machine. Reads are schema-inspected, read-only, parameterized, availability-aware, and source/timestamp wrapped. Pause/resume are PAPER-only, token protected, campaign-ID checked against the freshly-read sole active campaign, drift/recovery guarded, per-campaign locked, executed as argument lists without a shell, audited with redaction, and verified by a post-command canonical read. Resume additionally requires verified worker health; otherwise it returns partial failure.

Changed files are `dashboard/control_center.py`, `dashboard/app.py`, `tests/test_control_center_api.py`, `docs/CONTROL_CENTER_RUNTIME_MAPPING.md`, `docs/KOMUTLAR.md`, `VERSION.md`, `REPORT.md`, and `CHANGELOG.md`. No schema/migration, trading, signal, risk, lifecycle, export, or frontend file changed. Persistence reads do not write; the only additive persistence is a sanitized bounded operation JSONL audit under the existing burn-in artifact root. Compatibility handling returns `UNAVAILABLE_IN_SCHEMA` for optional evidence tables/fields; required campaign/run identity schema fails closed. No migration is required.

Tests cover active/no-active selection, null zero-denominator rates, ID injection, old optional schema, malformed preflight JSON, conservative worker health, exact CLI argument-list/no-shell behavior, nonzero command failure, drift/recovery resume gating, operation locking, log redaction, absent stop, and PAPER-only enforcement. Dashboard regression tests are also executed. Risks remain: no live Windows database/worker was available here, PID command-line identity is not canonically persisted, JSONL durability is filesystem-dependent, and real pause/resume must be manually validated during an approved PAPER window. Push is recommended only after the documented Windows read-first and controlled pause/resume acceptance steps pass; this does not authorize LIVE trading.

## 2026-08-01 — PR #311 merge-blocker correction

Root causes were page-limited reject semantics, pause verification that checked only the DB campaign status, and process-local operation locking without owner-safe cross-process lease release. Reject history now queries only canonical rejected observations, deduplicates by `observation_id` in a SQL CTE before campaign-wide count/reason aggregation and page limit, and explicitly falls back to raw-row semantics when historical schema lacks that ID. Missing reasons, explicit `UNKNOWN`, and malformed metrics JSON remain separate. Pending labels remain excluded.

Pause now records previous campaign/worker evidence, polls the canonical campaign PID/process and active continuation state for a bounded configurable period, and returns partial failure for live or ambiguous workers. A random operation-owned directory lease complements the thread lock; stale takeover and release use rename plus owner revalidation, and malformed metadata fails closed. No schema, migration, frontend, trading, signal, risk, or execution changes were made. Tests cover totals above the page size, pre-limit duplicate removal, full-page behavior, distribution scope/data quality, legacy identity fallback, pending-label isolation, pause success/partial/timeout/error paths, and lease ownership/takeover corruption. Live Windows acceptance remains outstanding and LIVE remains unauthorized.

Executed checks: `PYTHONPATH=src pytest -q tests/test_control_center_api.py tests/test_dashboard_app.py tests/test_phase9_burnin_ops.py tests/test_phase8_burnin_campaign.py tests/test_schema.py tests/test_sqlite_schema_bootstrap.py` completed with 197 passed, 3 skipped, and 0 failed; the skips are existing environment/schema-platform skips. After the final historical-schema ordering correction, `PYTHONPATH=src pytest -q tests/test_control_center_api.py` completed with 26 passed and 0 failed. `python -m compileall -q src/alphaforge/dashboard tests/test_control_center_api.py` and `git diff --check` passed. No live Windows campaign or worker was available, so no real pause/resume was issued.

## 2026-08-06 — PR #311 deployment compatibility patch

The backend implementation was preserved. The deployment gap was caused by absent frontend route aliases, no explicit cross-origin policy, no dedicated executable module, and health conflating database accessibility with service/runtime evidence. Minimal aliases now share the canonical handlers; exact-origin CORS is configurable without wildcard credentials; the executable module delegates to the existing `dashboard.app.create_app`; health and diagnostics separate database, campaign, worker, runtime, and control availability; and execution mode comes from typed AlphaForge configuration while mutations remain PAPER-only.

No frontend, schema, lifecycle, persistence, trading, signal, risk, or execution logic changed. The repository contains only the existing same-origin HTML dashboard and its partial refresh script, not the separately deployed Control Center SPA. Its actual base-URL configuration, response parsing, retry policy, and error mapping therefore cannot be verified here and require deployed frontend acceptance. Mixed-content and Private Network Access policy remain external browser constraints. No migration is required; compatibility risk is limited to operators needing to explicitly allow any non-default frontend origin.

Verification completed with `PYTHONPATH=src pytest -q tests/test_control_center_api.py tests/test_dashboard_app.py tests/test_phase9_burnin_ops.py tests/test_phase8_burnin_campaign.py tests/test_schema.py tests/test_sqlite_schema_bootstrap.py`: 203 passed, 3 skipped, 0 failed. The skips are pre-existing environment/schema-platform cases. `python -m compileall -q src/alphaforge/dashboard src/alphaforge/control_center.py tests/test_control_center_api.py`, `PYTHONPATH=src python -m alphaforge.control_center --help`, and `git diff --check` also passed. No live Windows worker or external frontend build was available.
