...
Added regression coverage for `KeyboardInterrupt`, `SystemExit`, worker early exit during attachment polling, zero-exposure failed startup terminalization, and symbol parser compatibility. Executed `pytest -q tests/test_phase9_burnin_ops.py` with 45 passing tests.
