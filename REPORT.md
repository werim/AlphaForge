...
`src/alphaforge/runtime.py` gives the attached campaign/run identity precedence over caller-supplied reject metadata. Fourteen P2-A fixtures now carry frozen market input across accepted outcomes, ambiguous closure, execution gates, correlated portfolio exposure, incomplete closed-candle geometry and resolved reject quality. `tests/test_issue421_full_chain_replay.py` drives real AIBrain scoring, PAPER runtime, persisted lifecycle/decisions, resolver eligibility/outcomes, readiness and isolated append-only audit ingestion twice per scenario. Eligible low-effective-RR rejects resolve with the production expected-fill basis: stop-first becomes `SL_BEFORE_TP` with negative net R and target-first becomes `TP_BEFORE_SL` with positive net R. Both remain exactly linked to the canonical reject and ingest as authoritative reject-shadow outcomes. Incomplete geometry remains resolver-ineligible. Existing ambiguous, SIGKILL cold-start and cross-campaign identity regressions remain.

## Validation / impact

Fourteen replay scenarios and 57 affected replay, resolver and audit tests passed. Accepted TP/SL paths preserve decision-to-position lineage and authoritative complete outcomes; ambiguous closure remains non-authoritative. Resolved reject outcomes preserve canonical label identity and audit authority; incomplete geometry is durable and auditable but resolver-ineligible. Readiness remains blocked. No schema/CSV shape, migration, threshold, cost, LIVE authorization or active campaign database change. Previously persisted historical rows are untouched. The remaining 9 golden scenarios and bounded load/release qualification still need production replay. Push recommendation: CHATGPT only, followed by exact-SHA CI; LIVE NOT READY.
