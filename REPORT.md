...
The P2-A golden pack declared full-chain expectations but exercised only a pre-submit projection. Initial frozen replays exposed campaign identity, accepted replay-finality and geometry evidence gaps; `489f495` repaired them and passed exact-head CI. The next identity check found that a caller-supplied `campaign_id`/`runtime_identity` in a reject payload could still override the attached runtime identity in review evidence. The pack also lacked production-chain proof for SHORT fills, losing accepted trades, ambiguous TP/SL closures and key execution rejects. The reject fixtures contained stale gate names and evidence semantics: missing liquidity evidence was labeled as missing price geometry, execution rejects were labeled as portfolio blocks, and thin liquidity omitted the preserved AIBrain primary reason.

## Change / behavior

`src/alphaforge/runtime.py` gives the attached campaign/run identity precedence over caller-supplied reject metadata. The first P2-D package already made that identity canonical for reject IDs, labels and observations, retained accepted geometry status, and scoped finalized replay to durable run/campaign evidence. Ten P2-A fixtures now carry frozen market input: profitable LONG/SHORT, losing LONG, ambiguous TP+SL, high spread, high slippage, thin liquidity, low effective RR, unavailable execution context and combined high-spread/low-effective-RR. `tests/test_issue421_full_chain_replay.py` drives real AIBrain scoring, PAPER runtime, persisted lifecycle/decisions, resolver, readiness and isolated append-only audit ingestion twice per scenario. Reject replays preserve complete canonical geometry, omit a false portfolio-block state, and create campaign-scoped pending reject labels without order execution. Thin liquidity records `LOW_EXECUTION_PROBABILITY` as the canonical primary while retaining `THIN_LIQUIDITY` in complete multi-gate evidence; high slippage uses the production `SLIPPAGE_TOO_HIGH` code; the combined case records spread as primary and retains both failed gates. The ambiguous resolver conservatively selects the stop, marks evidence incomplete, and the audit keeps its outcome non-authoritative. Existing real SIGKILL cold-start and cross-campaign identity regressions remain.

## Validation / impact

Ten replay tests and 142 focused/adjacent execution tests passed. Accepted TP/SL paths preserve decision-to-position lineage and authoritative complete outcomes; ambiguous closure has `evidence_complete=0` and `authoritative=0`. Spread, slippage, liquidity, low-effective-RR and unavailable-context rejects persist final decisions and campaign-scoped pending labels without execution. Readiness remains blocked. No schema/CSV shape, migration, threshold, cost, LIVE authorization or active campaign database change. Previously persisted historical rows are untouched. The remaining 13 golden scenarios and bounded load/release qualification still need production replay. Push recommendation: CHATGPT only, followed by exact-SHA CI; LIVE NOT READY.
...
The first sandboxed public SOAK probe returned no market rows solely because network access was restricted; the same read-only scanner returned 30 rows with approved public network access. That newly started qualification was ended through its own automatic teardown, leaving campaign, run, and mapping `COMPLETED` with an explicit `QUALIFICATION_AUTOMATIC_TEARDOWN` reason. The public run in `/private/tmp/alphaforge-autonomous-qualification/alphaforge-qualification-wr1q370p` was also terminalized through its own teardown after fifteen minutes of healthy samples, so a projected resource-gate false failure could be corrected before a full-duration run.

The SOAK audit exposed a release-gate gap: five-minute probes could not establish 120-second heartbeat freshness or resource stability. `src/alphaforge/autonomous_qualification.py` now samples the existing reconciliation, resolver, heartbeat, snapshot, campaign heartbeat, and lineage paths every 30 seconds; public scans occur every 300 seconds. A bounded JSONL sample artifact and final report include actual duration, RSS high-water, database/artifact growth, queue/backlog depths, SQLite lock-retry exhaustions, and scan/reconciliation latency. The first fifteen minutes showed normal append-only audit DB growth near 90 KiB per sample; a fixed 64 MiB cap projected a false six-hour failure. The DB growth limit now scales with sample count at 160 KiB per sample, with a 64 MiB minimum. Failed continuous invariants or sustained abnormal growth produce `NEEDS_FIX`. FAST behavior and the POST363 production recovery state machine are unchanged. `tests/test_autonomous_qualification.py` adds regression assertions for continuous samples and growth detection. No schema or campaign DB migration was made.

The ordinary full-suite run under the new filesystem sandbox failed 32 tests because those tests wrote default runtime DB or relative artifacts outside writable roots. To avoid touching any historical or production DB, the current branch was copied into a new `/private/tmp` test checkout containing only tracked files plus the SOAK source and test changes. The prior full suite there passed 1,532 tests with 3 skips. FAST passed in another fresh isolated qualification workspace at `/private/tmp/alphaforge-autonomous-qualification/alphaforge-qualification-dwimw967`.

The complete public SOAK at `/private/tmp/alphaforge-autonomous-qualification/alphaforge-qualification-gp339ja9` lasted 21,602.509 seconds and returned `NEEDS_FIX` solely because two of 72 public market scans returned no rows. All 720 continuous safety samples passed; reconciliation was CLEAN on every sample, heartbeat age peaked near one second, queue/backlog peak was zero, RSS high-water did not grow, DB grew 11.87 MB, artifacts 0.66 MB, scan latency peaked 9.207 seconds, and SQLite lock retry exhaustion count was one from the injected contention. Fifteen scheduled faults produced the expected recovery/terminal outcomes; no lineage drift, persistence gap, unexplained worker exit, or resource-growth flag appeared. The original market-data gate treated any empty scan as terminal and recorded only a cumulative count, leaving no event timing or recovery evidence. The smallest harness-only fix persists each public market-data probe and recovery in existing campaign events, accepts a single isolated empty scan only when the next five-minute probe recovers and empty scans remain at or below 5%, and fails on consecutive or unresolved gaps. Because the scanner suppresses its underlying exception, empty probes remain classified `UNKNOWN`; no transport cause is fabricated. PAPER execution stays blocked when no market candidate exists. The 12 focused regressions pass, including isolated and sustained public feed gaps. No product runtime, schema, migration, or historical campaign DB changed. A new full six-hour public SOAK and updated full suite remain required before PASS.

The updated full suite subsequently passed 1,535 tests with 3 skips in the isolated test checkout, and a fresh FAST qualification returned PASS at `/private/tmp/alphaforge-autonomous-qualification/alphaforge-qualification-b5iul2s2`. The next real public six-hour SOAK ran from 2026-09-15 13:08:20 UTC to 19:08:22 UTC in `/private/tmp/alphaforge-autonomous-qualification/alphaforge-qualification-z54y1qgr` and returned **PASS** after 21,602.472 seconds. All 720 30-second safety samples passed. Reconciliation was CLEAN and resolver status OK on every sample; all 72 public scans were audited, with 71 returning 30 candidates each and one returning none. The empty scan at 18:23:30 UTC was explicitly recovered on the next scan at 18:28:26 UTC, with 296.206-second latency and campaign event ID 1437. No PAPER execution occurred on an empty scan.

The final RSS high-water rose 1,458,176 bytes to 81,264,640 bytes and plateaued; DB growth was 11,795,400 bytes against a 117,964,800-byte audit-aware budget; artifacts grew 672,075 bytes. Queue and pending resolver/reject backlog peaks were zero. Reconciliation latency averaged 0.004345 seconds and peaked at 0.054793 seconds; the 72 market scans averaged 3.835946 seconds and peaked at 10.146749 seconds. The injected SQLite contention left one `PERSISTENCE_FAILED` event before recovery. Fifteen scheduled fault scenarios passed; recovered transport cases completed within 0.065020 seconds, SQLite contention within 0.967874 seconds, and the expected auth/protocol and grace-expiry cases paused atomically. The final isolated DB had 18 campaign lineages, all terminal and compatible (15 `COMPLETED`, 3 `PAUSED`), with explicit reasons, zero unsafe `OPERATING` snapshots, nine `RECOVERED` runtime events, no duplicate unknown-state failure cycle, and no persistence gap. Reports and export checksums were readable and internally consistent. There were 18 documented worker starts and 18 documented exits, including the supported continuation scenario; no unexplained exit or restart occurred.

The SOAK gate supports a prospective new PAPER burn-in qualification decision; it does not authorize LIVE readiness or start a campaign. Remaining limitations are that the public scanner suppresses the cause of an empty result (`UNKNOWN`), signed-account reconciliation in the harness remains deterministic rather than credentialed, RSS high-water does not prove every transient allocation is reclaimed, and an indefinitely unavailable SQLite writer delays durable audit evidence until recovery. No historical campaign DB, including POST363, was accessed or modified. No schema or migration change was made. The branch is reviewable for a small harness-only release-gate improvement.
...
## Why and root cause
POST363's signed `positionRisk` read became `EXCHANGE_STATE_UNKNOWN` after two failed `gaierror` attempts. Execution was fail-closed, but the independent candle resolver accumulated three `ProviderFailure:URLError` cycles at its 30-second cadence and hardcoded threshold, paused the campaign, and caused a clean worker shutdown after roughly 77 seconds. The resolver threshold ignored transport transience; watchdog counts also accumulated historical provider failures. Earlier heartbeat and snapshot writers could label the blocked runtime `OPERATING`.

## Files and state-machine change
`src/alphaforge/provider_failures.py` classifies explicit transport, auth/protocol, and unknown failures. `src/alphaforge/binance_reconciliation_provider.py` stores the class with the read-only evidence. `src/alphaforge/runtime.py` and `src/alphaforge/burnin_campaign.py` keep known transient failures in `RECOVERY_REQUIRED` with execution blocked while both loops continue. The attached resolver signals runtime failure and recovery-pending state; an idle resolver performs a real read-only candle probe before claiming recovery. `RuntimeOrchestrator._execute()` rechecks the fail-closed gate immediately before PAPER fill and records a cancellation for an in-flight decision that became unsafe. Auth/protocol failures pause immediately; unknown failures use the existing three-attempt escalation. The one new policy, `ALPHAFORGE_PROVIDER_TRANSIENT_OUTAGE_GRACE_SECONDS`, defaults to 300 and is wired through the config registry, loader, runtime builder, burn-in CLI/ops, and example environments.

`src/alphaforge/runtime_heartbeat.py` accepts `RECOVERY_REQUIRED`. `src/alphaforge/burnin_ops.py` counts active resolver/provider failures only since the last successful batch or recovery event and treats transient attempts inside grace as degraded, without unrelated historical escalation. The existing sustained backlog watchdog is held during an identified transient grace; unrelated hard-safety signals remain active.

## Persistence, lifecycle, export, and compatibility
Every signed request attempt remains in `exchange_reconciliation_events` diagnostics. Each resolver failure gets a unique attempt ID in `burnin_campaign_events`; attempts blocked by a temporary SQLite writer lock are queued for the next successful resolver or terminal transaction. `src/alphaforge/runtime_state.py` writes `RECOVERED` to the existing `runtime_recovery_events` table in the same transaction as the CLEAN reconciliation event and snapshot. Unknown state and resolver recovery-pending state clear only after that transaction commits. On expiry, `pause_campaign_for_provider_failure()` uses the existing terminalizer to update `burnin_campaigns`, `burnin_runs`, and `burnin_campaign_runs` atomically with an outage-specific reason. Finalization no longer overwrites a paused run as completed. No schema, export, or migration change was made; POST363.db was neither read nor modified in implementation, and no campaign/process was restarted.

BACKTEST behavior and reject persistence are unchanged. PAPER lifecycle gains an explicit `CANCELLED` outcome for a decision blocked at final execution. Active PAPER exposure remains unresolved during unknown exchange state. LIVE authorization is unchanged and LIVE remains not ready.

## Tests and remaining risk
`tests/test_provider_outage_recovery.py` covers transport/auth/unknown classification, single and repeated failures, attached resolver-to-runtime gating, idle recovery probing, committed CLEAN recovery after SQLite contention, complete-but-dirty evidence, unique audit records for identical provider payloads, atomic grace expiry, permanent auth escalation, active exposure, the final PAPER boundary, and watchdog reset. Existing reconciliation-contention, burn-in qualification/readability, reject persistence, runtime, heartbeat, and campaign supervision tests are retained. The final focused runtime/burn-in regression set passed 332 tests; the final full suite passed 1,514, skipped 3, and had the same nine previously documented backtest/trade-quality failures. `git diff --check` passed.

An indefinitely unavailable SQLite writer cannot make new evidence durable until storage recovers; queued resolver and reconciliation attempts remain fail-closed in memory meanwhile. A clean test suite does not establish seven-day worker stability, so deployment should use a fresh isolated PAPER burn-in and preserve POST363 as historical evidence. Push recommendation: review and merge the small operational patch after focused tests pass; do not use it to justify LIVE trading.

# Adaptive Decision Calibration Engine shadow foundation — 2026-09-14

## Why and root cause
Reject-forward expectancy differs across symbol/regime/side, while accepted trade outcomes and rejected forward outcomes were not analyzed in one decision-scoped calibration framework. A global reject percentage or threshold would miss cost-adjusted differences and could loosen unsafe gates. This patch creates an offline, retrospective shadow analysis boundary only.

The read-only architecture audit found accepted closed-trade net R in `burnin_trade_outcomes` (linked by `burnin_pending_position_outcomes`) and reject-forward net R in `burnin_reject_outcomes` (linked by `burnin_pending_reject_labels`). `burnin_observations` supplies canonical decision identity. Other gated decisions are observable, but without a valid pending forward label they have no attributable counterfactual outcome. `burnin_qualification.confidence_interval` and `burnin.canonical_decision_sql` are reusable; `burnin_campaign.execution_threshold_calibration` is reject-only, and `burnin_calibration_metrics` measures probability calibration rather than decision net R, so neither can be the common aggregator. `order.compute_adaptive_thresholds` already changes score/raw-RR thresholds from TP/SL streaks in the shared decision-quality path; this patch leaves that existing behavior unchanged and does not feed it shadow data.

## Files, behavior, and schema
- `src/alphaforge/adaptive_decision_calibration.py` reads canonical `burnin_observations`, links rejected outcomes through the exact campaign/run/pending-label/reject-decision chain, and links accepted outcomes through a unique same-run `signal_id` → pending position → trade outcome chain. It reuses `canonical_decision_sql`, `canonical_hash`, and `confidence_interval`. Non-attributable/orphan outcomes remain diagnostic, while incomplete/cancelled/ambiguous/execution-invalidated outcomes cannot improve calibration expectancy.
- One separate output SQLite database holds immutable versioned `adaptive_decision_observations`, current `adaptive_decision_calibration` snapshots, immutable `adaptive_decision_calibration_events`, and retrospective `adaptive_shadow_decisions`. Scope fallback is reason+symbol+regime+side → reason+symbol+regime → reason+regime → reason. There is no Cartesian scope expansion.
- Eligible net R drives mean, total, EWMA, recent/long expectancy, and 99% normal-approximation confidence bounds. TP/SL/ambiguous/timeout counts are diagnostic. At least 30 eligible samples, 10 recent samples, concordant recent/long signs, and a confidence bound clear of zero are required. Any incomplete or non-attributable row in a positive scope freezes relaxation. Missing or degraded execution, volatility, drawdown, or evidence-completeness health freezes relaxation. Negative evidence may tighten. Opposite-state hysteresis, 24-hour cooldown, 0.02 score/0.05 effective-RR maximum step, and fixed bounds around the base are applied to shadow proposals only.
- Numeric shadow proposals cover `MIN_TRADE_SCORE` in PAPER's 0–1 units and `MIN_EFFECTIVE_RR` for matching score/RR gates. MTF execution and counter-regime reasons can be measured but receive no numeric override. A rejected signal may only be marked as passing its calibrated gate with all other gates unverified; it is not labeled a safe trade. Existing execution, spread, slippage, liquidity, stop-width, portfolio, and exchange limits remain static safety boundaries.
- Candidate classification: score/effective-RR comparisons are `SAFE_FOR_SHADOW_CALIBRATION` because the shared quality function accepts a per-call configuration mapping. MTF direction/confirmation, counter-regime tolerance, and stop-width softening are `POTENTIALLY_ADAPTIVE_LATER` only after complete per-decision context and campaign identity accounting. Spread/slippage maxima, liquidity minimum, stop-width hard reject, portfolio exposure, daily loss, kill switch, and exchange-state checks are `STATIC_SAFETY_BOUNDARY`. The present patch changes none of them.
- `tests/test_adaptive_decision_calibration.py` uses isolated in-memory Phase 8 campaign tables and real evidence writers. No running DB, process, exchange, campaign backfill, qualification rule, or runtime decision code is touched.

## Lifecycle, persistence, compatibility, and migration
Accepted and rejected lifecycle behavior is unchanged. The shadow CLI opens its source with SQLite `mode=ro`, requires a different output path, and refuses an output containing campaign tables. The library rejects identical connections/files. Output observations are append-only by evidence hash; replay does not double-count a canonical decision. Snapshot rows update as new evidence arrives, with a separate immutable event for each changed evidence/state. The output schema is new and isolated, so production campaign schemas, SQL exports, and CSV contracts require no migration. Do not merge historical campaign evidence into a new qualification run; analysis is retrospective and in-sample. Shadow flips cannot be interpreted as counterfactual realized PnL, fill probability, or drawdown.

## Tests, limitations, and recommendation
The combined focused and adjacent runtime/burn-in regression set passed 316 tests (54 existing Python SQLite adapter deprecation warnings). It covers accepted/rejected net-R states, sample and confidence boundaries, diagnostic attribution, ambiguity, execution invalidation, system-health freezes, scope isolation/fallback, bounded steps, cooldown/hysteresis, replay, separate output, and unchanged shared BACKTEST/PAPER/LIVE quality decisions. Existing burn-in qualification and canonical-link tests remain authoritative and unchanged. Remaining gaps are causal out-of-sample evaluation, per-trade drawdown attribution, richer setup/score/execution buckets, proven health derivation from source evidence, and the existing streak-based actual-threshold heuristic. Review the isolated shadow patch; do not enable autonomous threshold mutation, push as a live-readiness claim, or start a campaign from this task.
...
- `src/alphaforge/lifecycle_contract.py`: keeps `ERROR` canonical but removes it from ordinary incoming lifecycle transitions; compatibility restart from audited error evidence remains explicit.
- `src/alphaforge/persistence.py`: requires every persisted `ERROR` to carry an explicit signal ID, failure reason, attempted state, and prior state. Empty or incomplete error evidence returns a clear persistence failure.
- `src/alphaforge/paper_burnin.py`, `src/alphaforge/burnin_qualification.py`, and `src/alphaforge/live_readiness.py`: classify attributed `ERROR` evidence as blocking rather than clean progress. Qualification derives `runtime_error_count` only from explicit run-scoped payload identity, avoiding unrelated historical evidence or cross-run signal reuse.
...
- Lifecycle, persistence, PAPER reporting, qualification, readiness, and health regressions cover identity continuity, same-symbol independent signals, complete/empty error metadata, blocking error classification, real write failure behavior, observation ordering, and campaign versus generic exposure tables.
...
The accepted PAPER chain is now `SIGNAL_CREATED -> WAITING_ENTRY_ZONE -> ENTRY_TRIGGERED -> ORDER_PLACED -> POSITION_OPENED` under one signal ID. A later signal on the same symbol begins with no prior state for its distinct ID and may reach `SIGNAL_REJECTED` without disturbing the open signal's chain. Validation is not disabled, invalid transitions are not swallowed, and genuine lifecycle database write failures still raise. `ERROR` uses existing lifecycle columns and JSON payload fields, requires complete audit metadata, increments lifecycle/runtime error evidence, and prevents qualification/readiness success. No schema migration or CSV-shape change is required. Existing runtime/campaign databases and historical rows are not modified.
...
The review-blocker follow-up focused lifecycle, runtime, persistence, PAPER report, qualification, readiness, and health suite passed 134 tests. The broader persistence, campaign, Phase 8/9, runtime-environment, LIVE-authorization, trading-mode, execution, parity, and JOB21 safety suite passed 271 tests. Both runs explicitly restored canonical test defaults (`MIN_EFFECTIVE_RR=1.6`, unknown expectancy blocked); 117 dependency deprecation warnings were non-failing. `git diff --check` passed.
...
- `src/alphaforge/runtime.py`: consumes only the explicit normalized MTF fields for AIBrain setup and momentum scoring, and consumes normalized regime alignment. It adds PAPER-only defaults of `$1,000` equity/available balance and `$10` candidate notional, records their provenance, and propagates the chosen notional into paper position accounting. Explicit market/account evidence still takes precedence. Setting either default to `None` preserves fail-closed behavior. Review also exposed a latent duplicate PAPER `ORDER_PLACED` emission; PAPER now leaves that event to `_execute()` after the simulated result, preventing the accepted lifecycle from degrading into `ERROR` states.
...
## Why the patch was needed and root cause
The pending-label persistence boundary accepted any non-null `reject_decision_id` and ran before the canonical burn-in `REJECTED` observation was written. Its existing unique constraints prevented duplicate strings but did not prove that the ID belonged to one canonical rejected decision in the same campaign/run. Runtime canonicalization trusted supplied IDs and timestamped some canonical observation keys, allowing namespace injection or duplicate observation artifacts. The resolver then trusted `rout_<reject_decision_id>` rows without explicit payload ownership, while campaign aggregation joined labels/outcomes by reject ID without run ownership. The read-only label report also allowed a NULL decision identity to match by `signal_id`. These paths let orphan, cross-run, diagnostic, collision, or retry artifacts enter operational label state and made qualification identity units diverge.

## Files changed and exact behavior
- `src/alphaforge/runtime.py`: persists the canonical reject observation in the same transaction before pending-label insertion. Reject IDs are deterministic hashes of campaign/run/signal/setup/symbol/decision context rather than `signal_id` alone; a differing caller-supplied ID is retained only as diagnostic provenance and cannot become canonical. Canonical reject observation keys derive from that identity rather than wall-clock time.
- `src/alphaforge/burnin_resolver.py`: validates campaign/run/canonical-reject ownership at the authoritative pending-label writer, derives the pending ID injectively from the canonical reject ID, preserves idempotent insert semantics, records invalid-link diagnostics, keeps prior geometry-ineligible decisions ineligible, revalidates identity before resolution, requires explicit pending-label/campaign/run ownership on existing outcomes, rejects outcome-ID-only conflicts, and removes signal-only review synchronization.
- `src/alphaforge/burnin_campaign.py`: computes distinct canonical decisions, labels, and outcomes on `(burnin_run_id, reject_decision_id)`, requires explicit pending-label/campaign/run linkage for qualification outcomes, scopes calibration joins by the same ownership chain, and carries verified pending linkage into materialized aggregate evidence.
- `src/alphaforge/burnin_qualification.py`: requires the explicit canonical ownership chain for identity-aware source-run outcomes and verified pending linkage for materialized aggregate outcomes.
- `src/alphaforge/reject_label_status.py`: uses canonical observation identity as the denominator, excludes invalid pending and outcome-ID-only rows from qualification coverage/quality, preserves them as integrity diagnostics, and removes signal-only identity inference.
- Focused and regression tests were added or updated in `tests/test_m0_canonical_reject_label_identity.py`, `tests/test_phase8_reject_resolver.py`, `tests/test_phase8_burnin_campaign.py`, `tests/test_burnin_qualification_evidence_integrity.py`, `tests/test_reject_calibration_canonical.py`, `tests/test_reject_label_status.py`, `tests/test_runtime.py`, and `tests/test_phase9_burnin_ops.py`.

## Lifecycle, persistence, export, compatibility, and migration
The canonical lifecycle remains `SIGNAL_CREATED -> SIGNAL_REJECTED`; only persistence ordering is tightened so the canonical reject exists before its pending label. Existing tables, columns, constraints, CSV shapes, geometry rules, resolver horizons, execution-cost math, scoring formulas, strategy gates, and PAPER/LIVE authorization are unchanged. No schema migration or historical repair is performed. Historical orphan rows remain diagnostic evidence and are excluded fail-closed when read or encountered by resolution.

## Tests, risks, limitations, and recommendation
The PR #356 CI follow-up restores the pre-existing repeated-reject contract: runtime supplies its internally derived canonical reject ID to AIBrain before scoring persistence, and restart lookup recognizes canonical setup observations within the same burn-in run. This prevents a provisional review plus canonical review pair and prevents duplicate callbacks after restart without using global signal-only suppression. The two exact CI regressions pass, the expanded focused identity set passes 14 tests, and the full suite passes 1462 tests with 3 skips and zero failures.

Focused canonical identity/resolver/aggregate/status coverage verifies exactly-once pending labels, campaign/run namespacing, caller-ID rejection, transaction rollback/retry, orphan/diagnostic exclusion, missing/cross-campaign/cross-run failure, outcome-collision and replay safety, bounded distinct cardinality, and preserved geometry ineligibility. The final bounded identity, resolver, campaign, qualification, status, runtime, Phase 8/9, and PR #355 scoring-context regression run passed 305 tests with zero failures (68 dependency deprecation warnings). Static Python compilation and `git diff --check` also passed. All tests use temporary or in-memory SQLite persistence; no runtime worker, burn-in command, network service, environment file, process, or existing campaign database was accessed.

Remaining limitation: historical data is intentionally not repaired, and standalone pre-contract orphan rows fail closed instead of being relinked. Review is recommended after the bounded suite passes; LIVE remains NOT READY.
...
PR review found two remaining empty-set errors: zero canonical reject IDs selected the legacy fallback even for a current Phase 8 campaign, and pending rejects subtracted every physical outcome rather than canonically linked completions. Identity mode now uses existing phase and observation identity metadata. Phase 8 or explicitly identified evidence requires canonical linkage; only non-Phase-8 evidence with actual observations lacking all identity markers is proven legacy. Empty or ambiguous evidence fails closed.
...
Canonical pending outcomes now subtract only unique canonically linked completed identities. Qualification ambiguous counts use attributable canonical outcomes, while `diagnostic_ambiguous_rejected_forward_outcomes` retains the physical diagnostic count. Orphan outcomes therefore change neither canonical sample sufficiency nor the aggregate hash.
...
PR #343 review follow-up execution: the focused qualification/aggregate/calibration set passed 25 tests, and the same bounded related suite passed 300 tests with 36 deprecation warnings. The two added regressions cover zero canonical rejects with orphan guided outcomes and 13 canonical rejects with only five linked completions plus excess orphan outcomes. No migration or strategy change was introduced.
...
## Why and root cause
The reconciliation runtime wrote findings, exchange reconciliation events, and runtime state snapshots in separate transactions. A concurrent SQLite `BEGIN IMMEDIATE` writer could interrupt one write after an earlier write committed. The unhandled `database is locked` exception then ended `reconciliation_loop`, causing the runtime to treat a transient database lock as a fatal task and potentially fail its burn-in campaign.

## Files and behavior
- `src/alphaforge/runtime_state.py`: additively migrates `exchange_reconciliation_events.cycle_id` and creates a unique index. The reconciliation-only writer commits the event, findings, and state snapshot in one transaction. It retries only SQLite `SQLITE_BUSY`/`database is locked`, with rollback through transaction exit and a fresh transaction on each attempt. A duplicate cycle skips findings and snapshot inserts. Previously configured per-connection busy timeout is restored.
- `src/alphaforge/runtime.py`: hashes runtime instance/startup, mode, source observation, status, read-only status, and stable finding content into `recon:v1:<SHA-256>`. The risk and LIVE authorization gates remain closed until the write commits. Exhausted contention sets `RECONCILIATION_PERSISTENCE_FAILED`, logs the cycle and error, keeps the reconciliation task alive, and queues a uniquely keyed `PERSISTENCE_FAILED` exchange event for the next successful transaction. Successful persistence clears this specific failure.
- `tests/test_reconciliation_sqlite_contention.py`: file-backed two-connection transient and persistent `BEGIN IMMEDIATE` contention, task liveness, fail-closed trading, deferred failure evidence, cycle idempotency, and atomic rollback on a non-busy write error.
- `VERSION.md`, `REPORT.md`, `CHANGELOG.md`: operational contract and validation.

## Lifecycle, persistence, export, compatibility, and migration
No lifecycle, strategy, resolver, qualification, campaign threshold, order-submission, or CSV-export behavior changes. The event-table migration is additive: historical rows have NULL cycle IDs, and SQLite's unique index permits those legacy NULLs. New runtime reconciliation events have deterministic unique IDs. Event, findings, and corresponding state snapshot commit or roll back together. Other callers of the legacy event and finding writers retain their existing behavior. The new queued failure event is historical evidence, not a CLEAN result; it becomes durable only when SQLite accepts a later transaction.

## Tests, remaining risks, and push recommendation
Focused reconciliation/Phase 5 tests passed 25 tests. The full relevant reconciliation, runtime, persistence, LIVE-readiness, agent-shadow, database-doctor, and Phase 8 campaign regression set passed 320 tests (54 non-failing Python sqlite adapter deprecation warnings). The reconciliation lock budget is four attempts, a 750 ms total deadline, 50 ms busy wait per attempt, and 25/50/100 ms backoffs. A blocked connection checkout or non-SQLite database error is outside this retry policy. Persistent lock evidence remains in memory/logs until a later successful write; monitoring must treat `RECONCILIATION_PERSISTENCE_FAILED` as blocking. No existing campaign database, POST361.db, or runtime process was accessed. Push after review; do not infer LIVE readiness.
# Runtime reject decision identity correction — 2026-09-13

## Why and root cause
The runtime wrote its canonical, campaign/run-scoped `reject:<hash>` ID into rejected reviews, burn-in observations, and pending labels, but the final core artifact callback omitted `decision_id`. `save_rejected_decision_artifact` then defaulted `order_decisions.decision_id` to `<signal_id>:REJECTED`. The resolver already preserved the `reject:<hash>` ID in `burnin_reject_outcomes.payload_json` and `rout_<reject_decision_id>`. A `mode=ro` query of `POSTDBFX.db` for `camp_bd9dc5aeadac0b59` found 82 pending labels, all 82 linked to a same-run canonical observation, none linked to a final core decision ID, and all 82 marked `LEGACY_SCANNER_SHADOW_CANDIDATE` with `reject_quality_attributable=false`. Campaign qualification excludes those 82 by provenance independently of the core ID mismatch; this patch intentionally preserves that exclusion.

## Files and behavior
- `src/alphaforge/runtime.py`: the final reject callback passes the runtime's existing canonical `reject_decision_id` to the core artifact writer. The accepted-decision writer and the artifact writer's fallback for other callers are unchanged.
- `tests/test_runtime.py`: a temporary-DB regression persists the same reject twice through the actual runtime callback; verifies one matching final decision, review, and pending label; verifies one attributable campaign label and 1.0 coverage; resolves the label and verifies the outcome carries the same identity and a retry creates no duplicate.
- `VERSION.md`, `REPORT.md`, `CHANGELOG.md`: record the behavioral and compatibility boundary.

## Lifecycle, persistence, export, and migration
Lifecycle transitions, geometry and cost requirements, reject selection, and execution are unchanged. The core decision key for new runtime rejects changes from `<signal_id>:REJECTED` to the already-established `reject:<hash>`; existing uniqueness and upsert semantics still give one row per replayed decision. No column, index, or CSV contract changes. Existing campaign DBs are not rewritten. An offline backfill, if desired after a campaign stops, must verify a unique same-run signal-to-reject mapping, check for target-key collisions and references, update the core decision key transactionally, and recompute qualification without overriding non-attributable provenance. Ambiguous records must remain diagnostic.

## Validation, remaining limits, and recommendation
The focused runtime, canonical reject, resolver, and label-status set passed 121 tests; adjacent campaign, qualification, calibration, and Phase 9 tests passed 182. `git diff --check` passed. Historical split identities remain until an audited offline repair. Zero coverage caused by legacy-shadow or false-attributability provenance will remain zero by design. Keep the running campaign untouched; review this prospective patch and use a fresh isolated PAPER run before interpreting qualification. LIVE remains NOT READY.

## CI #1601 compatibility follow-up — 2026-09-14
The runtime persistence callback indexed `payload["reject_decision_id"]`, raising `KeyError` for direct callers that previously relied on `save_rejected_decision_artifact` to derive `<signal_id>:REJECTED`. `src/alphaforge/runtime.py` now passes `payload.get("reject_decision_id")`; canonical runtime payloads still supply `reject:<hash>`, while absent IDs retain the artifact writer fallback. `tests/test_runtime_env_config.py` now asserts that fallback explicitly; the existing `tests/test_runtime.py` canonical-link regression remains unchanged. There is no lifecycle, resolver, qualification, schema, export, migration, or running-campaign impact. The two targeted tests and the 123-test runtime/persistence focused suite passed. Full pytest completed with 1,498 passed, 3 skipped, and 9 failures in unchanged backtest trade-quality/threshold/parity paths. No push recommendation until the existing full-suite failures are separately resolved or accepted.
