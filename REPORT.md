...
## Dedicated persistent Telegram transport state
Telegram polling now accepts an optional `SQLiteTelegramTransportStateStore` created only from an explicit caller-supplied path. It creates two logically independent tables: `telegram_transport_state(state_key, state_value, updated_at)` for the monotonic `next_offset`, and `telegram_pending_deliveries(update_id, chat_id, response_text, status, failure_reason, retry_count, created_at, updated_at)` for sanitized response-only recovery. No bot token, capability, runtime configuration, DB path, campaign/run ID, username, traceback, or raw exception is stored. Offset values are canonical decimal text in SQLite and validated back to Python integers, preserving values outside SQLite's signed-integer range.

For an accepted command, replay authorization remains unchanged and the controller is called at most once. The transport then bounds/redacts the response and commits its pending row plus monotonic next offset in one `BEGIN IMMEDIATE` transaction before `sendMessage`. A failed transaction rolls back both and prevents sending. A successful send deletes the pending row in a second durable transaction; a failed send retains it and increments a retry counter. Rejected terminal updates persist only their offset. Malformed persisted offset or delivery rows fail closed before polling or sending.

`retry_pending_telegram_responses(bot_token, http_client, state_store)` has no adapter, replay store, controller, runtime config, or executor input. It validates persisted rows, restores the private in-memory delivery capability, and retries only `sendMessage`. Command execution is AT-MOST-ONCE. Response delivery is AT-LEAST-ONCE when caller-driven recovery runs: if Telegram accepts a send and the process stops before SQLite records delivery, the pending row survives and restart may send a duplicate response. The command is never executed again, so exactly-once response delivery is explicitly not claimed.

Changed implementation files are `src/alphaforge/remote_control/telegram_state.py`, `src/alphaforge/remote_control/telegram_transport.py`, and `src/alphaforge/remote_control/__init__.py`; focused persistence tests are in `tests/test_remote_control_telegram_state.py`. Documentation updates are in `VERSION.md`, `REPORT.md`, and `CHANGELOG.md`. There is no campaign/runtime schema migration, service bootstrap, webhook, command expansion, or strategy/lifecycle change. Tests use temporary SQLite files, fake HTTP, and fake executors only. The next smallest step is a caller-supplied bootstrap/runner composition that opens the dedicated replay and transport-state stores and performs recovery before polling, without installing a service.

## Long-poll acknowledgement and response-delivery hardening
The original transport advanced its in-memory offset before adapter/controller processing and represented a failed `sendMessage` only as a counter. Because replay protection had already claimed an accepted update, there was no response-only artifact to retry safely. The transport now separates three facts: an authorized command attempt is acknowledged after one controller call; its sanitized response is represented by a capability-backed delivery record; and a parseable Telegram update is acknowledged by advancing the batch offset only after it reaches a terminal rejected, delivered, or pending-delivery state.

`deliver_telegram_response()` accepts only transport-minted pending deliveries and has no adapter, replay-store, controller, configuration, or executor input. A failed send remains `PENDING` with the stable reason `SEND_MESSAGE_FAILED`; retrying it can perform only `sendMessage`. Unauthorized, duplicate, and malformed updates remain non-executable. Adapter/store exceptions fail closed, produce no response, and acknowledge a parseable update to avoid a poison loop. Controller exceptions are normalized to a fixed safe response and never expose exception text. Offset and pending delivery state remain in memory only; persistence is intentionally deferred.

Across the acknowledgement and persistent-state steps, no campaign/runtime schema, lifecycle, export, strategy, execution, or runtime integration changed; no migration is required. The current full remote-control suite passes 141 tests using temporary controller-owned replay and transport-state stores, fake HTTP, and fake executors. No campaign/runtime database, real subprocess, Telegram/network request, or AlphaForge command was accessed. The remaining delivery risk is the documented duplicate-response window between Telegram accepting a send and the local delivered acknowledgement committing.

## Telegram update identity consistency
The Bot API boundary reads numeric `update_id` values as Python integers and keeps polling offsets as integers. The adapter continues to normalize Telegram IDs to strings for replay keys, audit rows, and verified requests. `TelegramResponseDelivery.update_id` now uses that same normalized string type, matching the value assigned by `poll_telegram_once()` and preserving it unchanged through response-only retries. No replay key, audit schema, offset arithmetic, authorization behavior, or validation rule changed. Focused tests cover matching request/delivery types, exact retry identity, integers larger than signed 64-bit range, the existing accepted negative-ID behavior, invalid nonnumeric rejection, and unchanged duplicate suppression.
...
This follow-up adds the next boundary only: a Telegram controller path from accepted normalized request to normalized response for `HELP`, `STATUS`, and `HEALTH`. It keeps `REPORT`, `REJECTS`, `LABELS`, and `ERRORS` accepted by the parser for future work but rejected by the executor controller until handlers exist.

The hardening follow-up closes a bypass risk: a public `TelegramRemoteControlRequest` can still be constructed for compatibility/readability, but the execution controller no longer accepts it. Only the adapter can mint `VerifiedTelegramRemoteControlRequest`, which carries a private module capability after authorization, replay claim, and accepted audit insertion. The executor-adapter follow-up then remaps verified `STATUS` and `HEALTH` through the existing trusted remote-control runtime mapping before calling an injected executor.

The transport follow-up adds a production-capable but injectable long-poll boundary. It calls Telegram Bot API concepts `getUpdates` and `sendMessage` through a caller-supplied HTTP client, processes updates sequentially through the existing adapter/controller, and never implements its own authorization or command parsing.
...
`src/alphaforge/remote_control/telegram_controller.py` coordinates only verified Telegram requests. `HELP` returns the currently executable Telegram command list (`/status`, `/health`, `/help`) with no executor, DB, subprocess, or runtime access. `STATUS` and `HEALTH` require trusted caller-supplied `RemoteControlConfig` or trusted mapping fields and are translated through `map_remote_command()`. The injected executor receives only the predefined runtime command:
`STATUS`: `("burnin_ops", "--db", trusted_db, "status", "--campaign-id", trusted_campaign_id, "--run-id", trusted_run_id)`.
`HEALTH`: `("burnin_ops", "--db", trusted_db, "health", "--campaign-id", trusted_campaign_id, "--run-id", trusted_run_id)`.
Unverified public request instances, tampered argv, unsupported commands, missing/incomplete trusted config, missing executors, executor exceptions, and invalid executor results return normalized fail-closed `RemoteControlDispatchResult` values without traceback/internal path/secret disclosure.
...
`src/alphaforge/remote_control/telegram_transport.py` adds `poll_telegram_once()` and `poll_telegram_loop()`. `poll_telegram_once()` accepts a bot token, current offset, `TelegramRemoteControlConfig`, trusted `RemoteControlConfig`/mapping, replay store, injected executor, and injected HTTP client. It validates the `getUpdates` envelope, advances offset to each parseable processed `update_id + 1`, sends responses only to the verified request `chat_id`, bounds Telegram output, and redacts the bot token plus trusted DB/campaign/run values from outbound text. `poll_telegram_loop()` runs a bounded number of iterations and sleeps through an injected backoff function only after transport-level getUpdates failure.
...
`tests/test_remote_control_telegram_controller.py` covers `HELP` output, no-executor `HELP`, exactly-once fake executor calls for verified `STATUS` and `HEALTH`, exact trusted STATUS/HEALTH argv mapping, Telegram request data isolation from DB/campaign/run arguments, direct unverified request rejection before executor, duplicate update rejection before executor, unauthorized user/chat rejection before executor, unsupported command rejection before executor, missing config failure before executor, safe executor failure, Telegram argument rejection before controller execution, tampered argv rejection, no `shell` kwarg, and no subprocess/network use.

`tests/test_remote_control_telegram_transport.py` covers fake `getUpdates` to adapter/controller/executor/sendMessage flow for `/status` and `/health`, `/help` with no runtime executor call, unauthorized and duplicate updates with no executor/send response, malformed updates without loop crash, trusted argv isolation from Telegram data, offset carry-forward, token/config redaction, send target restricted to verified chat ID, malformed API responses, send failures, bounded injected backoff, and fake-only HTTP/network assertions.
