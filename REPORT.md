...
The P2-A golden pack declared full-chain expectations but exercised only a pre-submit projection. Initial frozen replays exposed campaign identity, accepted replay-finality and geometry evidence gaps; `489f495` repaired them and passed exact-head CI. The next identity check found that a caller-supplied `campaign_id`/`runtime_identity` in a reject payload could still override the attached runtime identity in review evidence. The pack also lacked production-chain proof for SHORT fills, losing accepted trades and ambiguous TP/SL closures.

## Change / behavior

`src/alphaforge/runtime.py` now gives the attached campaign/run identity precedence over caller-supplied reject metadata. The first P2-D package already made that identity canonical for reject IDs, labels and observations, retained accepted geometry status, and scoped finalized replay to durable run/campaign evidence. Five P2-A fixtures now carry frozen market input: profitable LONG/SHORT, losing LONG, ambiguous TP+SL and high spread. `tests/test_issue421_full_chain_replay.py` drives real AIBrain scoring, PAPER runtime, persisted lifecycle/decisions, resolver, readiness and isolated append-only audit ingestion twice per scenario. The ambiguous resolver conservatively selects the stop, marks evidence incomplete, and the audit keeps its outcome non-authoritative. Existing real SIGKILL cold-start and cross-campaign identity regressions remain.

## Validation / impact

Five replay tests and 147 related runtime, resolver, audit and golden tests passed. Accepted TP/SL paths preserve decision-to-position lineage and authoritative complete outcomes; ambiguous closure has `evidence_complete=0` and `authoritative=0`. The spread reject persists a final decision and campaign-scoped pending label without execution. Readiness remains blocked. No schema/CSV shape, migration, threshold, cost, LIVE authorization or active campaign database change. Previously persisted historical rows are untouched. The remaining 18 golden scenarios and bounded load/release qualification still need production replay. Push recommendation: CHATGPT only, followed by exact-SHA CI; LIVE NOT READY.
