...
Changed files are `src/alphaforge/remote_control/telegram_transport.py`, `src/alphaforge/remote_control/__init__.py`, `tests/test_remote_control_telegram_transport.py`, `VERSION.md`, `REPORT.md`, and `CHANGELOG.md`. No campaign/runtime schema, lifecycle, export, strategy, execution, or runtime integration changed; no migration is required. The full remote-control suite passes 127 tests using temporary controller-owned replay stores, fake HTTP, and fake executors. No campaign/runtime database, real subprocess, Telegram/network request, or AlphaForge command was accessed. Remaining risk is process loss of pending responses and offsets; the next step is a dedicated persistent transport-state abstraction independent of campaign/runtime storage.
