...
## 2026-08-01 — PR #311 Control Center production-risk correction

Root causes were a response-generation timestamp masquerading as observation time, incomplete optional-column validation, recovery derived only from campaign status, SQLite conversion limited to connect/schema paths, an in-process lock, run-name contamination inference, and a pending-label queue presented as reject history. The correction keeps the existing router and canonical CLI adapter while adding per-source UTC freshness with an explicit 120-second configurable threshold, safe timestamp/order allowlists, centralized execute/fetch error conversion, active-lineage runtime recovery evidence with unknown failing closed, atomic directory leases shared across Uvicorn processes, unavailable canonical contamination, and rejected-observation semantics with separately labelled pending queue counts.

Changed files are `dashboard/control_center.py`, `tests/test_control_center_api.py`, `docs/CONTROL_CENTER_RUNTIME_MAPPING.md`, `VERSION.md`, `REPORT.md`, and `CHANGELOG.md`. There is no schema migration, frontend change, runtime write, trading/lifecycle logic replacement, or export change. Compatibility risk is limited to clients that incorrectly assumed freshness was always non-null or treated pending labels as full reject history. No migration is required. Live Windows DB/process verification remains pending; push remains conditional on the documented read-first PAPER acceptance procedure and does not authorize LIVE.
