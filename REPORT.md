...
Phase 7 now participates in PAPER and LIVE_PRECHECK runtime evidence collection without changing strategy decisions, reject logic, risk gates, or order lifecycle behavior. PAPER/LIVE_PRECHECK create or resume release-scoped burn-in runs, persist final decision observations, keep rejected decisions pending until forward labels complete, write realized trade outcomes only from `POSITION_CLOSED` closure evidence, emit execution/drawdown metrics, reconcile run counters from SQL, and generate qualification snapshots periodically and at shutdown. LIVE_PRECHECK remains non-mutating; if Phase 7 persistence or qualification fails, the canary path fails closed and records STOPPING/suspension evidence. `ExecutionMode.LIVE` remains hard-disabled by the existing Phase 6 guard and cannot be enabled by a Phase 7 canary snapshot.
...
Runtime collection now writes canonical decisions, realized PAPER closure outcome evidence, and periodic runtime metrics, but richer venue-specific forward labels and calibrated probability outputs still depend on upstream model/exchange evidence. Calibration quality requires real probability outputs and must not fabricate probabilities from scores.
