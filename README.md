...
BACKTEST shares important scoring, decision, execution-cost and lifecycle primitives, but its complete orchestration is not yet one identical pre-submit boundary with PAPER/LIVE_PRECHECK. P0-A/P0-B/P0-C and P1-A from [#421](https://github.com/werim/AlphaForge/issues/421) are merged on current `dev`; the remaining cross-mode invariant and release-verification work starts at P1-B.
...
A historical FAST/SOAK result is not proof for a newer commit. Fresh exact-head qualification must be bound to the code/config under evaluation. [#421](https://github.com/werim/AlphaForge/issues/421) remains the full-system verification tracker; current `dev` already contains P0-A/P0-B/P0-C and P1-A, while later P1/P2 verification families remain open.

## Testing

The repository uses `pytest` for unit, persistence, integration-style and production-path regression coverage. Current tests include lifecycle/reject persistence, execution-cost semantics, production execution-safety gates, authoritative PAPER portfolio-risk state, accepted/rejected resolver integrity, invalid/stale/future market-time fail-closed behavior, reconciliation contention/recovery, and the autonomous FAST/SOAK qualification harness.
