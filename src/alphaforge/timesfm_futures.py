...
def decide_from_forecast(
    *,
    ...
    if symbol != SUPPORTED_SYMBOL or timeframe not in SUPPORTED_INTERVALS:
        return _reject(timestamp, symbol, timeframe, current_price, forecast, "UNSUPPORTED_MARKET")
    if current_price <= 0 or current_price != current_price:
        return _reject(timestamp, symbol, timeframe, current_price, forecast, "INVALID_CURRENT_PRICE")
    if forecast is None:
        return _reject(timestamp, symbol, timeframe, current_price, forecast, "INVALID_FORECAST")
    if forecast.horizon not in SUPPORTED_HORIZONS:
        return _reject(timestamp, symbol, timeframe, current_price, forecast, "UNSUPPORTED_HORIZON")
    ...
    if upside / current_price >= min_edge_pct and long_risk > 0:
        ...
        if rr >= min_expected_rr:
            return TimesFMDecision(timestamp, symbol, timeframe, current_price, forecast.p10, forecast.p50, forecast.p90, "LONG", current_price, forecast.p10, forecast.p90, rr, None)
        return _reject(timestamp, symbol, timeframe, current_price, forecast, "LOW_EFFECTIVE_RR")

    if downside / current_price >= min_edge_pct and short_risk > 0:
        rr = downside / short_risk
        if rr >= min_expected_rr:
            return TimesFMDecision(timestamp, symbol, timeframe, current_price, forecast.p10, forecast.p50, forecast.p90, "SHORT", current_price, forecast.p90, forecast.p10, rr, None)
        return _reject(timestamp, symbol, timeframe, current_price, forecast, "LOW_EFFECTIVE_RR")

    return _reject(timestamp, symbol, timeframe, current_price, forecast, "LOW_CONFIDENCE")


def _reject(timestamp: int, symbol: str, timeframe: str, current_price: float, forecast: QuantileForecast | None, reason: str) -> TimesFMDecision:
    return TimesFMDecision(timestamp, symbol, timeframe, current_price, None if forecast is None else forecast.p10, None if forecast is None else forecast.p50, None if forecast is None else forecast.p90, "NO_TRADE", None, None, None, None, reason)
    ...
    *,
    ...
    for idx in range(min_history - 1, len(ordered)):
        ...
        try:
            ...
            forecast = None
        decisions.append(decide_from_forecast(timestamp=current.timestamp, symbol=symbol, timeframe=timeframe, current_price=current.close, forecast=forecast))
    return decisions
...
def write_decision_log(path: Path, decisions: Iterable[TimesFMDecision]) -> None:
    ...
    fieldnames = ["timestamp", "symbol", "timeframe", "current_price", "forecast_p10", "forecast_p50", "forecast_p90", "side", "entry", "stop", "take_profit", "expected_rr", "rejection_reason"]
