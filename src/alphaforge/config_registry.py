...
CONFIG_REGISTRY: tuple[ConfigSetting, ...] = (
    ...
    _s("ALPHAFORGE_DATABASE_URL", "database_url", "str", "sqlite+pysqlite:///data/runtime/alphaforge_runtime.db", "Mode / Safety", MODES, "Runtime persistence database URL.", dashboard_editable=False, deprecated_aliases=("ALPHAFORGE_DB_URL", "DATABASE_URL")),
    ...
    _s("ALPHAFORGE_PAPER_EXECUTION_LATENCY_MS", "paper_execution_latency_ms", "float", 50.0, "Execution Cost Filters", ("PAPER",), "Explicit PAPER pre-submit execution-latency model assumption in milliseconds; never derived from market-data HTTP RTT.", 0.0),
    _s("ALPHAFORGE_REGIME_TIMEFRAME", "regime_timeframe", "str", "1h", "Learning", ("PAPER",), "Closed-candle regime timeframe.", consumed_by="alphaforge.runtime._build_runtime_from_env", behavioral_test="tests/test_multi_timeframe.py::test_perfect_long_and_short_alignment"),
    _s("ALPHAFORGE_SETUP_TIMEFRAME", "setup_timeframe", "str", "15m", "Learning", ("PAPER",), "Closed-candle setup timeframe.", consumed_by="alphaforge.runtime._build_runtime_from_env", behavioral_test="tests/test_multi_timeframe.py::test_perfect_long_and_short_alignment"),
    _s("ALPHAFORGE_EXECUTION_TIMEFRAME", "execution_timeframe", "str", "1m", "Learning", ("PAPER",), "Closed-candle execution timeframe.", consumed_by="alphaforge.runtime._build_runtime_from_env", behavioral_test="tests/test_multi_timeframe.py::test_perfect_long_and_short_alignment", deprecated_aliases=("ALPHAFORGE_PAPER_DECISION_TIMEFRAME",)),
    _s("ALPHAFORGE_MTF_GUIDED_SIGNAL_GENERATION_ENABLED", "mtf_guided_signal_generation_enabled", "bool", True, "Learning", ("PAPER",), "Let 1h regime select trade side, 15m classify setup phase, and 1m confirm timing; false retains legacy equality-veto behavior.", consumed_by="alphaforge.runtime._build_runtime_from_env", behavioral_test="tests/test_runtime_env_config.py::test_mtf_guided_signal_generation_env_controls_provider"),
    _s("MTF_EXECUTION_CONFIRMATION_MODE", "mtf_execution_confirmation_mode", "str", "ENFORCE", "Learning", ("PAPER",), "PAPER-only MTF execution-confirmation authority: ENFORCE rejects unconfirmed execution; SHADOW persists the counterfactual and continues through existing downstream gates.", consumed_by="alphaforge.runtime.RuntimeOrchestrator._process_symbol", behavioral_test="tests/test_multi_timeframe.py::test_mtf_execution_confirmation_shadow_preserves_downstream_authority"),
    ...
    _s("ALPHAFORGE_SETUP_DIRECTION_THRESHOLD", "setup_direction_threshold", "float", 0.0003, "Learning", ("PAPER",), "Setup-layer normalized MA delta required for direction.", 0.0, consumed_by="alphaforge.multi_timeframe.BinanceMTFProvider.build", behavioral_test="tests/test_reject_calibration_canonical.py::test_layer_thresholds_are_independent_and_identity_sensitive"),
    _s("ALPHAFORGE_EXECUTION_DIRECTION_THRESHOLD", "execution_direction_threshold", "float", 0.0005, "Learning", ("PAPER",), "Execution-layer normalized MA delta required for confirmation.", 0.0, consumed_by="alphaforge.multi_timeframe.BinanceMTFProvider.build", behavioral_test="tests/test_reject_calibration_canonical.py::test_layer_thresholds_are_independent_and_identity_sensitive"),
    _s("ALPHAFORGE_ENABLE_STATE_DIRECTION_RESOLUTION", "enable_state_direction_resolution", "bool", False, "Learning", ("PAPER",), "Allow closed 1h regime plus 15m setup state to resolve a confirmed 1m execution signal to LONG, SHORT, or NO_TRADE.", consumed_by="alphaforge.runtime.RuntimeOrchestrator._process_symbol", behavioral_test="tests/test_multi_timeframe.py::test_state_direction_resolution_can_flip_same_exec_signal_or_no_trade"),
    ...
    _s("ALPHAFORGE_BINANCE_MARKET_DATA_BASE_URL", "binance_market_data_base_url", "str", "https://fapi.binance.com", "Binance", ("PAPER", "BACKTEST"), "Public read-only Binance USD-M market-data endpoint used consistently by scanning, MTF, and burn-in outcome resolution.", dashboard_editable=False, consumed_by="alphaforge.exchange_market_scanner._scan_binance"),
