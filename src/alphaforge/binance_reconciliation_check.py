...
def run(symbols: list[str]) -> dict[str, object]:
    ...
    try:
        settings = load_reconciliation_settings(os.environ, require_websocket=False)
        provider = BinanceReadonlyReconciliationProvider(config=BinanceReadonlyReconciliationConfig(
            base_url=settings.base_url, api_key=settings.api_key, api_secret=settings.api_secret,
            recv_window_ms=settings.recv_window_ms, request_timeout_sec=settings.timeout_sec,
            trade_lookback_ms=settings.trade_lookback_ms,
        ), tracked_symbols=lambda: set(symbols))
