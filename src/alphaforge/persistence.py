...
def _execution_context_is_unavailable(execution_ctx: Mapping[str, Any] | None) -> bool:
...
def init_db(database_url: str | None = None) -> Engine:
    ...
    ddl = [
        ...
        CREATE TABLE IF NOT EXISTS decision_evidence (
            ...
            total_notional_exposure REAL,
            max_notional_exposure REAL,
            symbol_notional_exposure REAL,
            max_symbol_notional REAL,
            side_exposure_long REAL,
            side_exposure_short REAL,
            net_exposure REAL,
            gross_exposure REAL,
            ...
            consecutive_loss_count INTEGER,
            correlation_group TEXT,
            correlation_group_exposure REAL,
            correlated_position_count INTEGER,
            risk_flags TEXT,
            portfolio_reject_reason TEXT,
            portfolio_risk_state TEXT,
            portfolio_diagnostics_json TEXT,
            signal_id TEXT,
            order_id TEXT,
            position_id TEXT,
            lifecycle_id TEXT,
        ...
        *_timesfm_forecast_evidence_ddl(),
        ...
        "CREATE TABLE IF NOT EXISTS release_gate_snapshots (id INTEGER PRIMARY KEY AUTOINCREMENT, release_id TEXT NOT NULL, phase TEXT NOT NULL, status TEXT NOT NULL, generated_at TEXT NOT NULL, canary_ready INTEGER NOT NULL, rollback_verified INTEGER NOT NULL, runbook_verified INTEGER NOT NULL, operator_acknowledged INTEGER NOT NULL, mutation_attempt_count INTEGER, blocking_reasons TEXT NOT NULL, evidence_json TEXT NOT NULL)",
        "CREATE TABLE IF NOT EXISTS operator_acknowledgements (id INTEGER PRIMARY KEY AUTOINCREMENT, ack_id TEXT NOT NULL UNIQUE, release_id TEXT NOT NULL, phase TEXT NOT NULL, acknowledged_at TEXT NOT NULL, valid_until TEXT NOT NULL, operator_id TEXT, acknowledgement_text TEXT, evidence_json TEXT NOT NULL)",
...
def _ensure_sqlite_runtime_schema(conn: Any) -> None:
    required_columns: dict[str, list[tuple[str, str]]] = {
        "order_decisions": [
            ...
            ("portfolio_reject_reason", "portfolio_reject_reason TEXT"),
            ("portfolio_risk_state", "portfolio_risk_state TEXT"),
            ("portfolio_diagnostics_json", "portfolio_diagnostics_json TEXT"),
        ...
        "decision_evidence": [
            ...
            ("total_explicit_cost_pct", "total_explicit_cost_pct REAL"),
            ...
            ("portfolio_equity", "portfolio_equity REAL"), ("available_balance", "available_balance REAL"),
            ("open_position_count", "open_position_count INTEGER"), ("max_open_positions", "max_open_positions INTEGER"),
            ("total_notional_exposure", "total_notional_exposure REAL"), ("max_notional_exposure", "max_notional_exposure REAL"),
            ("symbol_notional_exposure", "symbol_notional_exposure REAL"), ("max_symbol_notional", "max_symbol_notional REAL"),
            ("side_exposure_long", "side_exposure_long REAL"), ("side_exposure_short", "side_exposure_short REAL"),
            ("net_exposure", "net_exposure REAL"), ("gross_exposure", "gross_exposure REAL"),
            ("daily_realized_pnl", "daily_realized_pnl REAL"), ("daily_loss_pct", "daily_loss_pct REAL"),
            ("max_daily_loss_pct", "max_daily_loss_pct REAL"), ("rolling_drawdown_pct", "rolling_drawdown_pct REAL"),
            ("consecutive_loss_count", "consecutive_loss_count INTEGER"), ("correlation_group", "correlation_group TEXT"),
            ("correlation_group_exposure", "correlation_group_exposure REAL"), ("correlated_position_count", "correlated_position_count INTEGER"),
            ("risk_flags", "risk_flags TEXT"), ("portfolio_reject_reason", "portfolio_reject_reason TEXT"),
            ("portfolio_risk_state", "portfolio_risk_state TEXT"), ("portfolio_diagnostics_json", "portfolio_diagnostics_json TEXT"),
...
def save_order_decision(session: Any, **decision: Any) -> Any:
    ...
    payload = {
        ...
        "execution_ctx_missing": 1 if bool(decision.get("execution_ctx_missing", _execution_context_is_unavailable(execution_ctx))) else 0,
    ...
    try:
        ...
        INSERT INTO order_decisions (
            ...
            funding_rate_pct, execution_regime, volatility_regime, portfolio_reject_reason, portfolio_risk_state,
            portfolio_diagnostics_json, risk_flags, input_snapshot_hash, no_submit_verified, parity_result, created_at, updated_at
        ) VALUES (
            :decision_id, :signal_id, :order_id, :symbol, :mode, :phase, :decision, :order_type, :confidence, :explanation,
            :reject_reason, :score, :rr, :effective_rr, :expectancy_bucket, :order_payload, :payload, :execution_ctx,
            :execution_ctx_missing, :expected_slippage_pct, :spread_pct, :latency_ms, :orderbook_imbalance,
        ...
        ON CONFLICT(decision_id) DO UPDATE SET
            ...
            portfolio_reject_reason=excluded.portfolio_reject_reason, portfolio_risk_state=excluded.portfolio_risk_state,
            portfolio_diagnostics_json=excluded.portfolio_diagnostics_json, risk_flags=excluded.risk_flags,
    ...
    """), {
        ...
        "portfolio_reject_reason": decision.get("portfolio_reject_reason"),
        "portfolio_risk_state": decision.get("portfolio_risk_state"),
        "portfolio_diagnostics_json": json.dumps(decision.get("portfolio_diagnostics") or decision.get("portfolio_diagnostics_json") or {}),
        "risk_flags": json.dumps(decision.get("risk_flags") or []),
...
def save_rejected_decision_artifact(session: Any, **artifact: Any) -> dict[str, Any] | None:
    ...
    execution_ctx_missing = bool(
        artifact.get(
            ...
            _execution_context_is_unavailable(execution_ctx),
        )
    )
    signal_payload = {
        "signal_id": signal_id,
    ...
    decision_id = save_order_decision(
        ...
        portfolio_reject_reason=artifact.get("portfolio_reject_reason"),
        portfolio_risk_state=artifact.get("portfolio_risk_state"),
        portfolio_diagnostics=artifact.get("portfolio_diagnostics"),
...
def save_trade_lifecycle_event(session: Any, **event: Any) -> Any:
    ...
    payload = {
        ...
        "execution_ctx_missing": 1 if bool(event.get("execution_ctx_missing", _execution_context_is_unavailable(event.get("execution_ctx")))) else 0, "event_ts": canonical_utc_timestamp(event.get("event_ts")), "created_at": now,
