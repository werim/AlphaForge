...
class BurnInQualificationEngine:
    ...
    def evaluate(self, burnin_run_id: str) -> BurnInQualificationSnapshot:
        ...
        with self.engine.begin() as conn:
            ...
            obs_counts = conn.execute(text(f"SELECT SUM(CASE WHEN UPPER(COALESCE(decision,''))='ACCEPTED' THEN 1 ELSE 0 END) AS accepted, SUM(CASE WHEN UPPER(COALESCE(decision,''))='REJECTED' THEN 1 ELSE 0 END) AS rejected, COUNT(*) AS samples FROM burnin_observations o WHERE burnin_run_id=:id AND {canonical_decision_sql("o")}"), {"id": burnin_run_id}).mappings().first() or {}
