...
def _build_runtime_from_env() -> RuntimeOrchestrator:
    mode = execution_mode_from_env(os.getenv("ALPHAFORGE_EXECUTION_MODE") or os.getenv("EXECUTION_MODE"))
    database_url = os.getenv("ALPHAFORGE_DATABASE_URL") or os.getenv("ALPHAFORGE_DB_URL") or "sqlite+pysqlite:///:memory:"
    engine = init_db(database_url)
    brain = AIBrain(
        Session(engine),
        min_accept_score=_float_env_alias("ALPHAFORGE_MIN_SIGNAL_SCORE", 0.62, "ALPHAFORGE_MIN_ACCEPT_SCORE"),
    )
    config = RuntimeConfig(
        execution_mode=mode,
        ...
        max_concurrent_positions=_int_env_alias("ALPHAFORGE_MAX_CONCURRENT_POSITIONS", 3, "ALPHAFORGE_MAX_OPEN_POSITIONS"),
...
async def main() -> None:
    ...
    try:
        ...
        logger.info("runtime_shutdown mode=%s metrics=%s", orchestrator.metrics)
