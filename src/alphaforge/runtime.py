...
class RuntimeConfig:
    ...
    scanner_timeout_sec: float = 10.0
    max_reject_log_size: int = 500
...
class RuntimeOrchestrator:
    ...
    _reject_log: list[dict[str, Any]] = field(default_factory=list, init=False)
    _inflight_symbols: set[str] = field(default_factory=set, init=False)
    ...
    async def _scan_once(self) -> None:
        ...
        candidates = await asyncio.wait_for(self.market_scanner(), timeout=self.config.scanner_timeout_sec)
    ...
    async def _process_symbol(self, selection: SymbolSelectionResult) -> None:
        if selection.symbol in self._inflight_symbols:
            return
        self._inflight_symbols.add(selection.symbol)
        await self._emit_lifecycle_event("SIGNAL_CREATED", selection.symbol, {})
        try:
            await self._process_symbol_inner(selection)
        finally:
            self._inflight_symbols.discard(selection.symbol)

    async def _process_symbol_inner(self, selection: SymbolSelectionResult) -> None:
        ...
        score_ctx, order_plan, explanation = self.ai_brain.before_real_order(signal_payload, market_ctx, regime_ctx, stats_ctx)
    ...
    async def _persist_reject(self, payload: dict[str, Any]) -> None:
        ...
        if len(self._reject_log) > self.config.max_reject_log_size:
            del self._reject_log[:-self.config.max_reject_log_size]
    ...
    async def _heartbeat_loop(self) -> None:
        while not self._stop_event.is_set():
            ...
            try:
                await asyncio.wait_for(self._stop_event.wait(), timeout=self.config.heartbeat_interval_sec)
