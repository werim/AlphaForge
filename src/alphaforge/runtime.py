...
class RuntimeOrchestrator:
    ...
    async def _process_symbol(self, selection: SymbolSelectionResult) -> None:
        ...
        # TODO: before_real_order currently uses a sync SQLAlchemy session.
        # Keep this on the event-loop thread until session ownership is
        # refactored per decision (or moved behind an async-safe worker).
        ai_market_ctx = self._enrich_ai_market_context(market_ctx, signal_payload.get("execution_ctx", {}))
    ...
    @staticmethod
    def _enrich_ai_market_context(market_ctx: Mapping[str, Any], execution_ctx: Mapping[str, Any]) -> dict[str, Any]:
        ...
        for key in (
            ...
            if key not in enriched and key in execution_ctx:
