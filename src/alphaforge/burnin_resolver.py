...
from alphaforge.burnin import canonical_hash, persist_burnin_reject_outcome, persist_burnin_trade_outcome, utc_now, CRITICAL_COST_FIELDS
...
def persist_pending_reject_label(conn: Any, *, campaign_id: str, burnin_run_id: str, reject_decision_id: str, signal_id: str|None, symbol: str, side: str, decision_timestamp: str, entry: float, stop: float, target: float, horizon_seconds: float, execution_cost_assumptions: Mapping[str,Any], regime: str|None, reject_reason: str|None, source_provenance: Mapping[str,Any]) -> str:
    bootstrap_campaign_schema(conn); due_at=datetime.fromtimestamp(_dt(decision_timestamp).timestamp()+float(horizon_seconds), timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
