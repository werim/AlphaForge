...
def resolve_campaign_batch(conn: Any, campaign_id: str, candles_by_symbol: Mapping[str, Sequence[Mapping[str,Any]]] | Sequence[Mapping[str,Any]], *, now: str|None=None) -> dict[str,int]:
    bootstrap_campaign_schema(conn); now=now or utc_now(); rows=_exec(conn,"SELECT * FROM burnin_pending_reject_labels WHERE campaign_id=:cid AND status IN ('PENDING','READY') AND due_at <= :now ORDER BY decision_timestamp,id", {"cid": campaign_id, "now": now}).fetchall(); counts={"resolved":0,"pending":0,"ambiguous":0,"failed":0}
    for row in rows:
