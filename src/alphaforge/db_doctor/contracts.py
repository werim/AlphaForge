...
CONTRACTS={
"signals":c("core","REQUIRED",columns=("id","signal_id","symbol","side","timeframe","mode","created_at")),
"order_decisions":c("core","REQUIRED",columns=("id","decision_id","signal_id","decision","created_at")),
"ai_decision_features":c("core",columns=("id","decision_id","features","penalties","reason_flags","execution_features","created_at")),
...
TABLE:c("lifecycle","REQUIRED",columns=("id",)+tuple(sorted(WRITER_COLUMNS))),
...
"reconciliation_incidents":c("runtime","REQUIRED",columns=("id","incident_type","severity","symbol","lifecycle_ref","remediation_status","operator_acknowledged","fail_closed","forensic_payload","created_at"),owners=("RUNTIME_BOOTSTRAP",),sources=("src/alphaforge/reconciliation.py:ensure_reconciliation_tables",)),
}
...
# Columns for which each real writer constructs a non-NULL value itself.  The
# doctor uses this narrowly: merely naming a column in an INSERT is insufficient
# when the caller can still bind None.
WRITER_GUARANTEED_COLUMNS={
"persistence.save_signal":("signal_id","created_at","updated_at"),
"persistence.save_order_decision":("decision_id","order_payload","payload","execution_ctx","execution_ctx_missing","portfolio_diagnostics_json","risk_flags","no_submit_verified","created_at","updated_at"),
"persistence.save_trade_lifecycle_event":("event_id","signal_id","trade_id","lifecycle_state","state","event_type","payload","reject_reason","execution_ctx","execution_ctx_missing","event_ts","created_at","lifecycle_id","incident_payload"),
"runtime_heartbeat.record_runtime_heartbeat":("runtime_instance_id","execution_mode","heartbeat_ts","runtime_state","evidence_status"),
"runtime_state.persist_runtime_state":("timestamp","instance_id"),
"reconciliation.persist_incident":("incident_type","severity","symbol","lifecycle_ref","remediation_status","operator_acknowledged","fail_closed","forensic_payload"),
}
# These tables are SQL-first runtime contracts.  ORM classes with the same names
# are legacy query shapes, not independent schema owners and must not drive
# Alembic autogenerate.
SQL_FIRST_TABLES=frozenset({"signals","order_decisions","ai_decision_features",TABLE,"closed_trade_reviews","rejected_signal_reviews","setup_expectancy_stats","regime_expectancy_stats","symbol_expectancy_stats","cooldown_states"})
