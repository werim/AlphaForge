        ...
        if args.cmd=='worker':
            ...
            try:
                ...
                with engine.begin() as conn:
                    ...
                    existing=conn.execute(__import__("sqlalchemy").text("SELECT COUNT(*) FROM burnin_campaign_runs WHERE campaign_id=:cid"), {"cid": args.campaign_id}).scalar_one()
                    if not existing: start_or_resume_campaign(conn,args.campaign_id)
                if args.once:
