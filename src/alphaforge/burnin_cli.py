...
from alphaforge.burnin_campaign import create_campaign, start_or_resume_campaign, pause_campaign, get_campaign, qualify_campaign, export_campaign_bundle, bootstrap_campaign_schema, aggregate_campaign, BurnInCampaignRunner, recover_check_campaign, check_campaign_completion
...
def main(argv=None) -> int:
    ...
    for name in ('start','resume'):
        s=sub.add_parser(name); s.add_argument('--campaign-id',required=True); g=s.add_mutually_exclusive_group(); g.add_argument('--foreground', action='store_true'); g.add_argument('--detach', action='store_true')
    for name in ('status','pause','qualify','worker','resolve','complete','recover-check'):
    ...
    try:
        ...
        try:
            ...
            if args.cmd in {'start','resume'}:
                runner=BurnInCampaignRunner(create_engine(f"sqlite+pysqlite:///{db}",future=True), args.campaign_id)
                if args.detach:
                    res=runner.launch_detached(resume=args.cmd=='resume', db_path=db); _print(res,args.json); return 0 if res.get('status')=='RUNNING' else 1
                if args.foreground:
                    import asyncio
                    try:
                        res=asyncio.run(runner.run_foreground(resume=args.cmd=='resume')); _print(res,args.json); return 0
                    except KeyboardInterrupt:
                        _print({'status':'INTERRUPTED','campaign_id':args.campaign_id},args.json); return 130
                res=start_or_resume_campaign(conn,args.campaign_id,resume=args.cmd=='resume'); conn.commit(); _print(res,args.json); return 0
            if args.cmd=='worker':
                import asyncio
                engine=create_engine(f"sqlite+pysqlite:///{db}",future=True)
                try:
                    res=asyncio.run(BurnInCampaignRunner(engine,args.campaign_id).run_foreground(resume=False)); _print(res,args.json); return 0
                except KeyboardInterrupt:
                    _print({'status':'INTERRUPTED','campaign_id':args.campaign_id},args.json); return 130
                finally:
                    engine.dispose()
            if args.cmd=='resolve':
                ...
                res=resolve_campaign_batch(conn,args.campaign_id,{},now=__import__('alphaforge.burnin').burnin.utc_now()); conn.commit(); _print({'campaign_id':args.campaign_id,'resolver_counts':res},args.json); return 0
            if args.cmd=='complete': res=check_campaign_completion(conn,args.campaign_id); conn.commit(); _print(res,args.json); return 0 if res.get('complete') else 3
            if args.cmd=='recover-check': res=recover_check_campaign(conn,args.campaign_id); conn.commit(); _print(res,args.json); return 0
