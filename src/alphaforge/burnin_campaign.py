...
class BurnInCampaignRunner:
    """Minimal operational campaign worker loop for resolver progress without enabling LIVE."""
    def __init__(self, engine: Engine, campaign_id: str, candle_provider: Any, *, resolver_failure_threshold: int = 3, thresholds: BurnInThresholds | None = None) -> None:
        self.engine = engine; self.campaign_id = campaign_id; self.candle_provider = candle_provider; self.resolver_failure_threshold = resolver_failure_threshold; self.thresholds = thresholds; self.resolver_failure_count = 0
