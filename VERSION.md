...
- Current phase: twelve P2-A golden cases replay through real PAPER scoring/decision, lifecycle and SQLite persistence, resolver, readiness and isolated audit ingestion. The first ten passed exact-head CI on `ae70b73`.
...
- Lifecycle/execution/persistence: profitable LONG/SHORT, losing LONG, ambiguous TP+SL, execution gates, correlated portfolio exposure and incomplete closed-candle geometry replay with stable semantics. Reject evidence retains canonical geometry and durable campaign-scoped labels when eligible; incomplete geometry remains canonical decision evidence but cannot enter the forward-label queue. Thin liquidity preserves both the AIBrain primary reason and the hard execution gate; the combined case preserves the complete gate set with spread as the canonical primary. Portfolio evidence records the concrete `CORRELATION_OVEREXPOSURE` state. Ambiguous intrabar closure remains evidence-incomplete and non-authoritative in audit. A real SIGKILL cold-start position blocks replay. PAPER fills remain modeled. No schema/export migration or historical evidence rewrite.
- Validation: twelve replay scenarios and 50 affected replay/portfolio/geometry tests passed; exact-head CI for this extension remains required. Known risks: 11 other golden cases still lack production full-chain replay; P2-E load, P2-F documentation sweep and P1-E release qualification remain. Last audit: 2026-09-24. LIVE NOT READY.
