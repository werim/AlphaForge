...
- Current phase: fourteen P2-A golden cases replay through real PAPER scoring/decision, lifecycle and SQLite persistence, resolver, readiness and isolated audit ingestion. The first twelve passed exact-head CI on `f86fa06`.
...
- Lifecycle/execution/persistence: profitable LONG/SHORT, losing LONG, ambiguous TP+SL, execution gates, correlated portfolio exposure, incomplete closed-candle geometry, and correct/false reject outcomes replay with stable semantics. Eligible rejects resolve from expected-fill geometry: stop-first is a correct reject and target-first is a false reject, with authoritative audit outcomes. Incomplete geometry remains canonical decision evidence but cannot enter the forward-label queue. Portfolio evidence records the concrete `CORRELATION_OVEREXPOSURE` state. Ambiguous intrabar closure remains evidence-incomplete and non-authoritative in audit. A real SIGKILL cold-start position blocks replay. PAPER fills remain modeled. No schema/export migration or historical evidence rewrite.
- Validation: fourteen replay scenarios and 57 affected replay/resolver/audit tests passed; exact-head CI for this extension remains required. Known risks: 9 other golden cases still lack production full-chain replay; P2-E load, P2-F documentation sweep and P1-E release qualification remain. Last audit: 2026-09-24. LIVE NOT READY.
