...
- Current version: feature/354-remote-control Phase-3 Telegram parser, authorization, replay, audit, and HELP/STATUS/HEALTH executor boundary; no polling/webhook/network transport.
- Current phase: read-only remote-control transport hardening.
- Runtime maturity: Telegram updates can normalize only `/status`, `/health`, `/report`, `/rejects`, `/labels`, `/errors`, and `/help` into predefined internal command names. `HELP` is answered without runtime access; `STATUS` and `HEALTH` require an injected executor. The adapter performs no execution.
