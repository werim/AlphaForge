...
- Runtime maturity: PAPER reject labels are created only after an explicit canonical `REJECTED` observation exists in the same campaign/run; canonical IDs are internally namespaced, and retries retain one pending label and one explicitly ownership-linked immutable outcome identity.
