# #421 P2-D initial full-chain replay — 2026-09-24

### Added
- Frozen profitable-LONG and high-spread PAPER replays across decision, persistence, resolver, readiness and isolated audit, each repeated for deterministic semantics; real SIGKILL cold-start replay protection.
### Changed
- Attached campaign identity takes precedence in runtime reject and decision evidence; accepted decision JSON retains geometry status/reason; finalized PAPER lookup recognizes durable accepted evidence and scopes rejects to the current campaign/run.
### Fixed
- Reject forward labels escaping an attached campaign without an environment campaign ID; repeated processing of a durable accepted PAPER signal; a final reject from another campaign falsely suppressing a reused signal; stale `decision_evidence` writer guidance and two golden expectations.
### Removed
- None.
### Breaking Changes
...
### Known Issues
- Only two of 23 golden cases run the production full chain. P2-E load and P1-E release qualification remain; LIVE NOT READY.
...
### Fixed
- Campaign health now resolves the current attachment runtime instance and never borrows heartbeat or scan metrics from another PAPER runtime.
...
- Alembic revision tests import the required dependency directly instead of silently skipping executable migration coverage.
...
## Known Issues
- PostgreSQL migration acceptance still requires a configured PostgreSQL integration environment; incompatible pre-existing table shapes require explicit repair and are not rewritten automatically.
...
## Changed
- Phase 8 attachment now records three identity views (campaign, active run, runtime), their mismatch maps, and identity sources. `git_commit` is validated campaign-to-run as persisted provenance; execution-cost identity remains campaign-scoped because `burnin_runs` has no such column.
...
- Cost-adjusted qualification engine using conservative lower confidence bound expectancy, reject-quality value, calibration quality, drawdown/loss-cluster checks, execution degradation status, concentration limits, canonical Phase 1-6 release controls, operator acknowledgement, full-test evidence, and runtime reconciliation evidence.
